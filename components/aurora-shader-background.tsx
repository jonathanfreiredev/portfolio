...
const UNIFORMS = {
  colors: [
    [0.0392156862745098, 0.0392156862745098, 0.07058823529411765],
    [0.3568627450980392, 0.16470588235294117, 0.5254901960784314],
    [0.8392156862745098, 0.30980392156862746, 0.6588235294117647],
    [1, 0.7607843137254902, 0.8862745098039215],
    [1, 0.7607843137254902, 0.8862745098039215],
    [1, 0.7607843137254902, 0.8862745098039215],
    [1, 0.7607843137254902, 0.8862745098039215],
    [1, 0.7607843137254902, 0.8862745098039215],
...
type AuroraShaderBackgroundProps = {
  ...
  /** Velocidad de la animación. Default: UNIFORMS.timeScale (-0.271). Negativo invierte la dirección; 0 la detiene. */
  speed?: number;
  /** Paleta de colores en [R,G,B] 0–1. Hasta 8. Default: paleta original del shader. */
  colors?: RGB[];
};
...
export function AuroraShaderBackground({
  ...
  className,
  style,
  speed = UNIFORMS.timeScale,
  colors = UNIFORMS.colors,
  ...
  useEffect(() => {
    ...
    while (paddedColors.length < 8) {
      paddedColors.push(colors[colors.length - 1]);
    }
    ...
    const resizeCanvas = () => {
      ...
      const w = Math.max(1, Math.round(rawWidth * pixelScale));
      const h = Math.max(1, Math.round(rawHeight * pixelScale));
